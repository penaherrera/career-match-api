...
export default function DashboardPage() {
  const [activeItem, setActiveItem] = useState("bienvenido");
  const location = useLocation();

  useEffect(() => {
    if (location.pathname.includes('test')) {
      setActiveItem('iniciar-prueba');
    }
  }, [location]);
    ...
    <DashboardLayout>
      <div className="flex h-screen">
        ...
        <div className="hidden md:block w-64 h-full">
          <Sidebar onNavigate={setActiveItem} activeItem={activeItem} />
        </div>
        {/* Main Content */}
        <div className="flex-1">
          <div className="md:hidden">
            <MobileSidebar>
              <Sidebar className="px-2" onNavigate={setActiveItem} activeItem={activeItem} />
            </MobileSidebar>
          </div>
          {renderContent()}
        </div>
